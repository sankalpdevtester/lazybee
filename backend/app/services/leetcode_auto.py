...
async def run_daily_leetcode(num_problems: int = 26):
    ...
    log(f"Session: {len(session)} chars, starts {session[:10]}...")
    ...
    try:
        ...
        log(f"Queue: {len(queue)} unsolved, targeting {num_problems} accepted")
        ...
        log(f"Session done: {submitted}/{num_problems} submitted")
